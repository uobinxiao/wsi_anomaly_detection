...
class LinearVVAttention(nn.Module):
    def __init__(self, dim, num_heads=8, qkv_bias=False, qk_scale=None, attn_drop=0., proj_drop=0.):
        super().__init__()
        self.num_heads = num_heads
        head_dim = dim // num_heads
...
class SwitchLinearAttention(nn.Module):
    def __init__(self, dim, num_heads=8, qkv_bias=False, qk_scale=None, attn_drop=0., proj_drop=0., top_k = 1, num_experts = 1, apply_noise = False):
        super().__init__()
        self.num_heads = num_heads
        head_dim = dim // num_heads
        self.scale = qk_scale or head_dim ** -0.5
        ...
        if self.num_experts == 1:
            ...
            experts = nn.ModuleList([nn.Linear(dim, dim) for _ in range(num_experts)])
            self.moe = SparseMoE(n_embed = dim, experts = experts, top_k = moe_top_k, apply_noise = apply_noise)
...
class Block(nn.Module):
    def __init__(self, dim, num_heads, mlp_ratio=4., qkv_bias=False, qk_scale=None, drop=0., attn_drop=0.,
                 drop_path=0., act_layer=nn.GELU, norm_layer=nn.LayerNorm, attn=Attention, num_experts = 1, moe_top_k = 1, apply_noise = False, attn_n_experts = 0):
        super().__init__()
...
class ConvBlock(nn.Module):
    def __init__(self, dim, kernel_size=3, mlp_ratio=4., drop=0.,
                 drop_path=0., act_layer=nn.GELU, norm_layer=nn.LayerNorm, num_experts = 1, moe_top_k = 1, apply_noise = False):
        super().__init__()
        self.norm1 = norm_layer(dim)
        ...
        if self.num_experts == 1:
            ...
            experts = nn.ModuleList([Mlp(in_features=dim, hidden_features=mlp_hidden_dim, act_layer=act_layer, drop=drop) for _ in range(num_experts)])
            self.moe = SparseMoE(n_embed = dim, experts = experts, top_k = moe_top_k, apply_noise=apply_noise)
    ...
    def forward(self, x, return_attention=False, attn_mask=None):
        ...
        if self.num_experts == 1:
            ...
            moe_x, gating_logits = self.moe(self.norm2(x))
            x = x + self.drop_path(moe_x)
        if return_attention:
            return x, None
        else:
            return x, gating_logits
