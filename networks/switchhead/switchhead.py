...
class SwitchHeadCore(torch.nn.Module):
    def __init__(self, d_model: int, num_heads: int, n_experts: int, dropout: float = 0.0,
        ...
        self.projection_size = d_head or (d_model // num_heads)

        self.q = torch.nn.Linear(self.input_size, self.projection_size * self.num_heads, bias=False)
        self.k = torch.nn.Linear(self.input_size, self.projection_size * self.num_heads, bias=False)

        if self.n_experts > 1:
            self.v = torch.nn.Parameter(torch.empty(self.num_heads * self.n_experts, self.input_size, self.projection_size))
            self.o = torch.nn.Parameter(torch.empty(self.num_heads * self.n_experts, self.projection_size, self.output_size))
            self.sel_v = torch.nn.Parameter(torch.empty(self.num_heads * self.n_experts, self.input_size))
        else:
            self.v = torch.nn.Parameter(torch.empty(self.num_heads * self.projection_size, self.input_size))
            self.o = torch.nn.Parameter(torch.empty(self.output_size, self.num_heads * self.projection_size))
    ...
    def reset_parameters(self, init_scale: float):
        ...
        torch.nn.init.normal_(self.o, 0, init_scale / math.sqrt(self.num_heads * self.projection_size))
...
class SwitchHeadRope(SwitchHeadCore):
    def __init__(self, d_model: int, num_heads: int, n_experts: int, dropout: float = 0.0,
                 d_head: Optional[int] = None, expert_dropout: float = 0.0, moe_k: int = 2,
                 init_scale: float = 1.0, rotate_fraction: float = 0.5, rope_base: float = 10000, qkv_bias = False, qk_scale = 0, attn_drop = 0, proj_drop = 0):

        super().__init__(
            d_model, num_heads, n_experts, dropout, d_head, expert_dropout, moe_k)
...
class SwitchHeadXL(SwitchHeadCore):
    def __init__(self, d_model: int, num_heads: int, n_experts: int, dropout: float = 0.0,
        ...
        self.pos_to_pk = torch.nn.Parameter(torch.empty(num_heads * self.projection_size, self.pe_size))
