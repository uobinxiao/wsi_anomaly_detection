...
class ViTill(nn.Module):
    def __init__(
        ...
        if hasattr(self.encoder, "num_reg_tokens"):
            self.encoder.num_register_tokens = self.encoder.num_reg_tokens
        elif not hasattr(self.encoder, 'num_register_tokens'):
            self.encoder.num_register_tokens = 0

        self.film_in = FiLMCondition(cond_dim=embed_dim, feat_dim=embed_dim)
        self.film_dec = nn.ModuleList([
            FiLMCondition(cond_dim=embed_dim, feat_dim=embed_dim) for _ in range(len(self.decoder))
            ])
    ...
    def forward(self, x):
        ...
        condition = x[:, : 1 + self.encoder.num_register_tokens, :]
